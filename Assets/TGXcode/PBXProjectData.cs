    ...
    internal class PBXProjectData
    {
        ...
        void RefreshMapsForGroupChildren(string projectPath, string realPath, PBXSourceTree realPathTree, PBXGroupData parent)
        {
            ...
            foreach (string guid in children)
            {
                ...
                if (fileRef != null)
                {
					pPath = UnityEditor.iOS.Xcode.PBX.Utils.CombinePaths(projectPath, fileRef.name);
					UnityEditor.iOS.Xcode.PBX.Utils.CombinePaths(realPath, realPathTree, fileRef.path, fileRef.tree, out rPath, out rTree);
                ...
                if (gr != null)
                {
					pPath = UnityEditor.iOS.Xcode.PBX.Utils.CombinePaths(projectPath, gr.name);
					UnityEditor.iOS.Xcode.PBX.Utils.CombinePaths(realPath, realPathTree, gr.path, gr.tree, out rPath, out rTree);
